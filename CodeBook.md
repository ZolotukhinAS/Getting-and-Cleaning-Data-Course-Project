...
Data located in 2 files in required format:<return>
*data.txt	-	**Combined dataset**. Contains merged original datasets (test+train) with subset of variables
...
Data was merged from two source datasets (train and test) which were in common structure by definition.

####key attributes
* observation 	(int)	- 	number of observation(experiment) in source dataset
* dataset	(chr) 	-	"test"|"train". Lineage for source dataset information
* subjectid	(int)	-	1:30. Id of the subjects in experiment
* activity	(chr) 	-	different type of activities recorded by experiment.

####features' data
Following variables are subset of data source variables, only mean and standard deviation was saved.
Information in that variables is the same as it was presented in source databases.

They all have numeric datatypes. 
...
Data was calculated based on **Combined dataset**. It contains **averages** for all features' values grouped subjectid and activity. **Names of the variables were saved** even if it is now contains the avarage per subject | activity. 
...
6. Saving datasets into output txt files.
